...
var geodata=jsonKreise;

var height= 400;
var width_map = 500;
var fullwidth = 900 ;
var width_chart = fullwidth-width_map;
...
var b = path.bounds(geodata),
	s = .95 / Math.max((b[1][0] - b[0][0]) / width_map, (b[1][1] - b[0][1]) / height),
	t = [(width_map - s * (b[1][0] + b[0][0])) / 2, (height - s * (b[1][1] + b[0][1])) / 2];
...
var map = d3.select("#map").append("svg").attr("id","map").attr("width", width_map).attr("height", height);
map.append("rect")
	.attr("id", "rect")
...
var chart = d3.select("#map").append("svg").attr("id", "chart")
					.attr("width", fullwidth-width_map)
					.attr("height", height);
					
function buildChart(dataset, width_chart, height) {
	var xScale = d3.scale.linear()
			.domain([0, dataset.length])
			.range([padding, width_chart-padding])
	...
	chart.select("#plot").append("text")
		.attr("class", "x label")
		.attr("text-anchor", "end")
		.attr("x", width_chart - 42)
		.attr("y", height - 12)
		.text("Monat im Jahr");
